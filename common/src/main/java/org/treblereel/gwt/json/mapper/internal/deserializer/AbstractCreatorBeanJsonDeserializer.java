...
import jakarta.json.stream.JsonbCreatorParameterDeserializer;
...
public abstract class AbstractCreatorBeanJsonDeserializer<T> extends JsonbDeserializer<T>
  ...
  protected Map<String, JsonbCreatorParameterDeserializer> creatorParams = new LinkedHashMap();
    ...
    creatorParams.forEach(
        (key, deserializer) -> {
          if (jsonObjectDecorator.containsKey(key) && !jsonObjectDecorator.isNull(key)) {
            params.put(key, deserializer.deserialize(jsonObjectDecorator, ctx));
          }
        });
